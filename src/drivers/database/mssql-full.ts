...
// 1. Install the mssql package: npm install mssql @types/mssql
// 2. Replace the content of mssql.ts with this file
...
export class MSSQLQueryableWithPackage implements QueryableBaseDriver {
  private pool: any; // sql.ConnectionPool when mssql package is available
  private connected = false;

  constructor(private config: MSSQLConnectionConfig) {
    // Uncomment when mssql package is available:
    /*
    ...
    */
    throw new Error("This implementation requires the 'mssql' package to be installed");
  }
  ...
  private mapMSSQLResult(result: any): DatabaseResultSet {
    // Uncomment when mssql package is available:
    /*
  ...
  private mapMSSQLTypeToGeneric(sqlType: any): string {
    // Uncomment when mssql package is available:
    /*
